...
def get_s1_ts(lon, lat, polygon, ismn_idx, start, end, pol, mode, res, red, scale, crs, get_grid_scale, idx_name):
	"""
	...
	# Create Polygon Geometry (Longitude/Latitude)
	if polygon is not None:
		poi = ee.Geometry.Polygon(polygon) #GEE Geometry Object
		poi_fc = ee.FeatureCollection(poi)
	...
	try:
		...
		geometry = [Polygon(x['geometry']['coordinates'][0]) for x in ts['features']]
...
def get_s2_ts(lon, lat, polygon, ismn_idx, start, end, red, scale, crs, idx_name):
	"""
	Arguments:
	bla
	"""
...
def merge_s1_s2_era5(gdf_s1, gdf_s2, lon, lat, gdf_era5, driver, filepath, idx_name):
	"""
	Arguments:
	"""
	...
	try:
		...
		name = idx_name + '_' + str(gdf.iloc[0][idx_name]) + '_' + str(lon) + '_' + str(lat) + '.geojson'
...
def get_s1_s2_era5_df(longitudes, latitudes, polygon, ids, start, end, filepath, scale_s1, scale_s2, scale_era5, idx_name, get_grid_scale):
	"""
	Arguments: 
	"""
